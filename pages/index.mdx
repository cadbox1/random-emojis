...
1. Using Durable Objects with Pages isn't well documented.
1. No logs for Functions.
1. The Functions API is slightly different to Workers.
1. My first NextJS build failed.
1. Clicking on an in-progress build doesn't always take you to that build.
1. Pages builds were down one day.

None of these were deal breakers but they did subtract from the developer experience. Thankfully, I think a lot of these could be improved quite easily.
