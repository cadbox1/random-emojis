...
On a bit of a tangent, I also built an NPM package for my frontend components as part of this project. I've built NextJS apps with MDX and custom components [before](https://github.com/cadbox1/prawn-stack) and wanted to split them out into a reusable package instead of copying and pasting them. The result is [Cadell's Vanilla Components](https://github.com/cadbox1/cadells-vanilla-components) and [Cadell's NextJS template](https://github.com/cadbox1/cadells-nextjs-template). It was a bit harder than I thought it would be and wasn't strictly required for this project but I'm really happy with the result and it should make my next app much easier to build. For building the package, I tried using [tsdx](https://github.com/jaredpalmer/tsdx) but typescript complained with the types when I used the package and I couldn't work out why. Thankfully, I found [tsup](https://github.com/egoist/tsup) which pretty much worked out of the box. It also uses [esbuild](https://github.com/evanw/esbuild) instead of [Rollup](https://github.com/rollup/rollup) so that's cool too. tsdx introduced me to [np](https://github.com/sindresorhus/np) for actually publishing to NPM and it works really well so I'm still using it. Sometimes everything happens for a reason.
...
Unfortunately, new products and hype trains are often a bit rough around the edges and that was certainly my experience here. Again, I'm really happy with the result but it was a bit of journey to get here.
...
None of these are deal breakers for me but they did subtract from the developer experience. Thankfully, a lot of these should be easy to fix.
